...
//g++ -std=c++11 -pthread generateCalibrationMatrix.cpp -ldl -o /home/aspelun1/Documents/IncrementalLearning//generateCalibrationMatrix
...
int main(int argc, char * argv[])
{
   	...
   	if (argc !=2)
	{
		printf("Correct program call is \"./ProgramName xyz\"\nwhere \"xyz\" are participant initials.\n");
		return 1;
	}
	

	char outputFilename[128];
	strcpy(outputFilename,"/home/aspelun1/Documents/IncrementalLearning/Data/"); 
	strcat(outputFilename,argv[1]);
	
	struct stat sb;

	if (stat(outputFilename, &sb) == 0 && S_ISDIR(sb.st_mode))
	{
		printf("\n\nFolder exists.\n");
	}
	else
...
//	unsigned long int programTime = 0.0;
	...
	PCA::adjust_data(IMU_values, means);

	Array2D<double> covar_matrix( NUMBER_OF_IMUS*3,  NUMBER_OF_IMUS*3);
	PCA::compute_covariance_matrix(IMU_values, covar_matrix);
	...
	PCA::eigen(covar_matrix, eigenvector, eigenvalue);
	
	
	printf("Printing PCA data info to file \"Matrix_%s.txt\"\n",argv[1]);
