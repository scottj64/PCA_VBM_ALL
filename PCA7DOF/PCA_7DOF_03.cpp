...
// g++ -std=c++11 -pthread PCA_7DOF_03.cpp -ldl -o /home/aspelun1/Documents/IncrementalLearning/PCA_7DOF
...
//	printf("buf = %d %d %d %d %d %d %d %d\n",int(buf[0]),int(buf[1]),int(buf[2]),int(buf[3]),int(buf[4]),int(buf[5]),int(buf[6]),int(buf[7]));
	...
	if(canExecuteProgram)
	{
		...
		pointToSend.Position.Fingers.Finger3 = 6250.0f; //does this go more?
...
*/		currDP.fingerSVal = (data.Fingers.Finger1 + data.Fingers.Finger2 + data.Fingers.Finger3) / 3;
		...
		if (CartYOn) pointToSend.Position.CartesianPosition.Y = -currDP.sentValue[2];
		else pointToSend.Position.CartesianPosition.Y = -(START_Y - currDP.currentLoc[1]);
		if (CartZOn) pointToSend.Position.CartesianPosition.Z =  currDP.sentValue[1];
		else pointToSend.Position.CartesianPosition.Z = START_Z - currDP.currentLoc[2];
		
		if (RotXOn)	 pointToSend.Position.CartesianPosition.ThetaX = currDP.sentValue[4];//X;
		if (RotYOn)	 pointToSend.Position.CartesianPosition.ThetaY = currDP.sentValue[5];//Y;
		if (RotZOn)	 pointToSend.Position.CartesianPosition.ThetaZ = currDP.sentValue[6];//Z;
		if (GrasperOn)
		{
			pointToSend.Position.Fingers.Finger1 = 
			pointToSend.Position.Fingers.Finger2 = 
