...
// g++ -std=c++11 -pthread PCA_7DOF_04.cpp -ldl -o /home/aspelun1/Documents/IncrementalLearning/PCA_7DOF
...
#include "globalVarsAndConsts_7DOF_PCA_02.h"
...
int main(int argc, char * argv[])
{
	...
	pthread_t readerThread, teensyThread;
...
//	float parameters[7] = {F1Gain, F2Gain, F3Gain, F4Gain, F5Gain, F6Gain, F7Gain};
	...
	if(canExecuteProgram)
	{
		...
		pointToSend.Position.Fingers.Finger3 = 6250.0f; //does this go more? It does, to 6250 at least
...
*/		currDP.fingerSVal = (data.Fingers.Finger1 + data.Fingers.Finger2 + data.Fingers.Finger3) / 3;
		...
		else //NUM_OF_DOFs >=4
		{
			...
			*/
			...
			{
				if (currDP.currentLoc[0]<=0.25 &&currDP.currentLoc[1]<=0.45 &&currDP.currentLoc[2]>=0.4)
				{
					keepGoing = false;
				}
			}
		...
		{
			memset(currDP.sentValue, 0.0, 7);//sizeof(currDP.sentValue);
		}
		else
		{
		...
		//Boundary Checkin				
		if((currDP.currentLoc[0] - X_ROBOT_MIN) <= 0.0 && currDP.sentValue[0] <= 0.0)
		{
			currDP.sentValue[0] = 0.0;
			currDP.againstBorder = true;
		}
		if((currDP.currentLoc[0] - X_ROBOT_MAX) >= 0.0 && currDP.sentValue[0] >= 0.0)
		{
			currDP.sentValue[0] = 0.0;
			currDP.againstBorder = true;
		}
		if((currDP.currentLoc[1] - Y_ROBOT_MIN) <= 0.0 && currDP.sentValue[2] <= 0.0)
		{
			currDP.sentValue[2] = 0.0;
			currDP.againstBorder = true;
		}
		if((currDP.currentLoc[1] - Y_ROBOT_MAX) >= 0.0 && currDP.sentValue[1] >= 0.0)
		{
			currDP.sentValue[2] = 0.0;
			currDP.againstBorder = true;
		}
		if((currDP.currentLoc[2] - Z_ROBOT_MIN) <= 0.0 && currDP.sentValue[1] <= 0.0)
		{
			currDP.sentValue[1] = 0.0;
			currDP.againstBorder = true;
		}
		
		
		if (CartXOn)
			pointToSend.Position.CartesianPosition.X = -currDP.sentValue[0];
		else
			pointToSend.Position.CartesianPosition.X = -(START_X - currDP.currentLoc[0]);	
		if (CartYOn)
			pointToSend.Position.CartesianPosition.Y = -currDP.sentValue[2];
		else
			pointToSend.Position.CartesianPosition.Y = -(START_Y - currDP.currentLoc[1]);
		if (CartZOn)
			pointToSend.Position.CartesianPosition.Z = currDP.sentValue[1];
		else
			pointToSend.Position.CartesianPosition.Z = START_Z - currDP.currentLoc[2];
		
		if (RotXOn)
			pointToSend.Position.CartesianPosition.ThetaX = currDP.sentValue[4];//X;
		if (RotYOn)
			pointToSend.Position.CartesianPosition.ThetaY = currDP.sentValue[5];//Y;
		if (RotZOn)
			pointToSend.Position.CartesianPosition.ThetaZ = currDP.sentValue[6];//Z;
		if (GrasperOn)
		{
			pointToSend.Position.Fingers.Finger1 = 
			pointToSend.Position.Fingers.Finger2 = 
			pointToSend.Position.Fingers.Finger3 = (float){(currDP.sentValue[3] > 0.0) ? 1000.0:-1000.0} ;
		}
...
//	parameterFile.close();
	...
	printf ("Output file \"%s/PCA_Session_%dDOF_%d.txt\" closed.\n", argv[1], NUM_OF_DOFs, q);
