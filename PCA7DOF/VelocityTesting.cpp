...
//g++ -std=c++11 -pthread VelocityTesting.cpp -ldl -o VelocityTesting -lboost_iostreams -lboost_system -lboost_filesystem
...
*/	
	...
	float previousX, previousY, velocityX_meas, velocityY_meas, velocityMag_meas, velocityMag_sent;
	...
	while(keepGoing) //errors should break out of the loop and set keepGoing to false for the reader thread
   	{
		...
		//} gonna include all this other stuff in this else statement
			...
			{
				tempVelocityX    = currDP.velocityX - 0.5*(currDP.velocityX + currDP.velocityY) - boundaryVelocityBias;
				currDP.velocityY = currDP.velocityY - 0.5*(currDP.velocityX + currDP.velocityY) - boundaryVelocityBias;
				currDP.velocityX = tempVelocityX;
			}
		...
		velocityX_meas = (currDP.currentX-previousX)*LOOP_FREQUENCY;
		velocityY_meas = (currDP.currentY-previousY)*LOOP_FREQUENCY;
		velocityMag_meas = sqrt(velocityX_meas*velocityX_meas + velocityY_meas*velocityY_meas);
		
		velocityMag_sent = sqrt(currDP.velocityX*currDP.velocityX +currDP.velocityY*currDP.velocityY);
		
		printf("%f\t%f\n",velocityMag_sent,velocityMag_meas);
		
		
		previousX = currDP.currentX;
		previousY = currDP.currentY;
